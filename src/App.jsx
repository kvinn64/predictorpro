...
// Has this match already kicked off?
function hasStarted(day, kickoff) {
  const k = kickoffDate(day, kickoff);
  if (!k) return false; // unknown time → treat as still open
  return Date.now() >= k.getTime();
}
// Are ALL matches in a day's list already kicked off?
function allStarted(day, list) {
  if (!list || list.length === 0) return false;
  return list.every((m) => hasStarted(day, m.kickoff));
}
...
// Sends the Supabase access token so the server can identify the user and
// enforce a per-user daily rate limit (protects your API credit).
async function callOpus(prompt, maxTokens = 1200) {
...
export default function App({ session }) {
  ...
  const todayIdx = 1; // buildDays: index 0 = yesterday, 1 = today, 2 = +1, 3 = +2
  const [activeDay, setActiveDay] = useState(days[todayIdx]);
  const [userMovedDay, setUserMovedDay] = useState(false);
  ...
  // Day rollover: once ALL of today's matches have kicked off, the default
  // view moves to tomorrow (unless the user has manually picked a day).
  useEffect(() => {
    if (!hydrated || userMovedDay || rolledOver) return;
    const today = days[todayIdx];
    const todayList = fixtures[dayKey(today)];
    if (todayList && todayList.length > 0 && allStarted(today, todayList)) {
      setActiveDay(days[todayIdx + 1]); // tomorrow
      setRolledOver(true);
    }
    // eslint-disable-next-line
  }, [hydrated, fixtures, userMovedDay, rolledOver]);
...
Return up to 10 matches, ordered by kickoff. Use 24-hour HH:MM for kickoff. If you cannot find any fixtures for that date, return [].`;
  ...
  async function openMatch(m) {
    ...
    // Already have your private prediction? Show it (free), even after kickoff.
    if (predictions[id]) {
    ...
    // Otherwise: pre-match only. Once it's kicked off, no new analysis.
    if (hasStarted(activeDay, m.kickoff)) {
      setSelected(m); setSelectedId(id); setDetailTab("analysis");
      setDetail({ locked: true });
...
Use web search to find recent form, injuries, suspensions, likely lineups, head-to-head, tactical setup, venue and stakes before deciding. Then respond with ONLY valid JSON, no markdown, no backticks:
...
Provide 4-6 analysis sections and 4-6 picks across different markets. Be specific and grounded in what you found.`;
    try {
      const d = await callOpus(prompt, 1500);
      setDetail(d);
    ...
    <div style={{ fontFamily: "'Inter',-apple-system,system-ui,sans-serif", background: BG, color: TEXT, minHeight: "100vh", maxWidth: 460, margin: "0 auto", paddingBottom: 90, position: "relative" }}>
      ...
      {page === "home" && (<>
        <div style={{ display: "flex", gap: 9, padding: "8px 14px 0", overflowX: "auto" }}>
          {days.map((d) => {
            ...
            return (
              <button key={dayKey(d)} onClick={() => { setActiveDay(d); setUserMovedDay(true); }}
        ...
        <div style={{ padding: "0 14px" }}>
          ...
          {!loadingDay && list.map((m, i) => {
            ...
            const locked = started && !analyzed; // can't analyze after kickoff
            return (
              <div key={i} className="fade" onClick={() => openMatch(m)}
                style={{ background: CARD, border: `1px solid ${analyzed ? ACCENT_DIM : BORDER}`, borderRadius: 18, marginBottom: 13, cursor: locked ? "not-allowed" : "pointer", overflow: "hidden", opacity: locked ? 0.55 : 1 }}>
                  ...
                  {filter === "all" && (
                    <div style={{ flex: 1, textAlign: "right", borderLeft: `1px solid ${BORDER}`, paddingLeft: 12 }}>
                      {analyzed && !predictions[id].error ? (<>
                        ...
                        <div style={{ color: MUTED, fontWeight: 700, fontSize: 13, lineHeight: 1.3 }}>{locked ? "kicked off" : "tap to analyze"}</div>
                      )}
                    </div>
                  )}
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 11, padding: "11px 16px", borderTop: `1px solid ${BORDER}`, background: "rgba(255,255,255,.015)" }}>
                  {started
                    ? <span style={{ background: "rgba(130,148,174,.16)", color: MUTED, fontWeight: 700, fontSize: 11, letterSpacing: 0.8, padding: "4px 10px", borderRadius: 7 }}>KICKED OFF</span>
                  ...
                  <span style={{ marginLeft: "auto", color: MUTED, fontSize: 13 }}>{locked ? "🔒" : analyzed ? "view ›" : "tap ›"}</span>
                </div>
              </div>
            );
          })}
        ...
        <div style={{ padding: "18px 18px 0", color: MUTED, fontSize: 11.5, lineHeight: 1.6 }}>
          Your predictions are private to your account and generated on demand by Claude Opus 4.8 — only before kickoff. For information and entertainment only — not betting advice.
        </div>
      </>)}
      ...
      {selected && (
        <Detail match={selected} detail={detail} loading={detailLoading} stage={detailStage}
          ...
          onClose={() => { setSelected(null); setSelectedId(null); setDetail(null); }} onRetry={() => openMatch(selected)} />
      )}
    </div>
  );
}
...
function Detail({ match, detail, loading, stage, tab, setTab, dayLabel, result, grade, onLog, onClose, onRetry }) {
  return (
    <div className="fade" style={{ position: "fixed", inset: 0, maxWidth: 460, margin: "0 auto", background: BG, zIndex: 50, overflowY: "auto", paddingBottom: 40 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ textAlign: "center" }}>
            <div style={{ fontSize: 44, fontWeight: 900, letterSpacing: -1, lineHeight: 1 }}>
              {detail && !detail.error && !detail.locked
      ...
      {!loading && detail && detail.locked && (
        <div style={{ padding: "50px 28px", textAlign: "center", color: MUTED }}>
          <div style={{ fontSize: 34, marginBottom: 12 }}>🔒</div>
          <div style={{ fontWeight: 700, fontSize: 16, color: TEXT }}>This match has kicked off</div>
          <div style={{ marginTop: 10, fontSize: 14, lineHeight: 1.6, maxWidth: 300, marginInline: "auto" }}>
            Predictions can only be generated before kickoff. You didn't analyze this one in time — but you can still log the final score from the Stats tab once it's played.
          </div>
          <button onClick={onClose} style={{ ...retryBtn, background: CARD_HI, color: TEXT }}>Close</button>
        </div>
      )}
      ...
      {!loading && detail && !detail.error && !detail.locked && (
        <div className="fade">
